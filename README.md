...
Run the graph. There is a color video preview in the graph that can be activated. It will slow down the conversion, but it's highly recommend to use it, as you can live check if the video flaps to pink at some point.
...
The n is a variable that should be toggled between 0 and 2, as by experience that formula seems to releate to field instead of frame. Theoretically it can be anything between zero and the last field of the video. It can be seen as a start-field option. So use this to find a start frame where both fields are not pink. (As said usually only 0 and 2 make sense)
